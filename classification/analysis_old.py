...
for gname in all_data["group"].unique():
    ...
    plt.bar(
        list(range(gsorted.shape[0])), gsorted[gname], width=1.0, color=colors
    )
    plt.savefig("certainty_{}.png".format(gname))
    plt.close()


xts = PCA(n_components=2).fit_transform(
    all_data.drop(["group", "infiltration", "corr", "pred"], axis=1)
)
...
overviewsp = {
    p: os.path.join(
        ...
        p,
        "overview_plots/avg_stats.csv"
    ) for p in os.listdir("output/classification")
    if "dedup" in p
}
...
overviews = {
    k.replace(
        "initial_comp_", ""
    ).split("_dedup_")[0]: pd.read_table(v, sep=",", index_col=0)
...
over_df.loc[
    over_df["name"].apply(lambda x: "all_groups" in x), "groups"
] = "all"
