...
def create_tsne(data: flowcat.utils.URLPath, meta: flowcat.utils.URLPath, plotdir: flowcat.utils.URLPath):
    ...
    labels = flowcat.io_functions.load_json(plotdir / "case_ids.json")
    ...
    for tube in ("1", "2", "3"):
        ...
        for case in cases:
            ...
            sample.path = data / f"{case.id}_t{tube}.csv"
            som = sample.get_data().data.values.flatten()
            soms.append(som)
        ...
        for group in flowcat.mappings.GROUPS:
